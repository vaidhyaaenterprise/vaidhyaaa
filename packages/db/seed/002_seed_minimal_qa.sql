...
-- Demo clinic (unique number 1000, matching clinic_unique_number_seq starting value)
INSERT INTO clinics (id, name, primary_phone, address_line1, city, state, country, timezone, default_language_code, active, onboarding_status, unique_number)
VALUES ('00000000-0000-0000-0000-000000000001', 'Sri Murugan Clinic', '+914400000001', 'Anna Nagar', 'Chennai', 'Tamil Nadu', 'India', 'Asia/Kolkata', 'ta_tanglish', true, 'ready_for_agent', 1000)
ON CONFLICT (id) DO NOTHING;

-- Keep the sequence ahead of the seeded unique number so future registrations never collide.
SELECT setval('clinic_unique_number_seq', 1000, true);
...
('00000000-0000-0000-0000-000000000001','MRI scan-ku fasting venuma?','MRI brain scans usually do not need fasting. Abdomen MRI may need 4-6 hours fasting — staff will confirm your exact scan type.','pre_visit_instruction','["mri fasting","fast before mri","mri scan preparation"]','approved','mri fasting fast before mri scan preparation abdomen', '00000000-0000-0000-0000-000000000102', now())
ON CONFLICT DO NOTHING;
